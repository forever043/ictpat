...
class PatentForm(forms.ModelForm):
	...
	class Meta:
		model = Patent
		fields = [ 'name', 'department', 'inventors', 'type', 'state',
			   'apply_code', 'apply_date', 'authorize_code', 'authorize_date', 'apply_file', 'authorize_file' ]
		widgets = {
	...
	def save(self, commit=True):
		...
		for extfield in self.extfields:
			...
			try:
				...
				PatentExtField.objects.filter(patent = instance, type = extfield_type).update(value=self.cleaned_data[extfield])
