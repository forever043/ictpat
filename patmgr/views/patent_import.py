...
class PatentImportWizard(SessionWizardView):
	form_list = FORMS
	file_storage = FileSystemStorage(location=os.path.join(settings.BASE_DIR, 'uploads'))

	def get_context_data(self, **kwargs):
		context = super(PatentImportWizard, self).get_context_data(**kwargs)
		...
		for bf in basefields:
			if bf in field_matchid:
				try:
					...
					break
				setattr(patent, bf, value)

	def do_update(self, row, basefields, extfields, field_matchid):
		patent = Patent()
		for bf in basefields:
			if bf in field_matchid:
				setattr(patent, bf, string.strip(row[field_matchid[bf]]))
		print patent
