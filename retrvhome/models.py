...
class PatentField(models.Model):
	...
	class Meta:
		verbose_name = u'检索专利字段'
		verbose_name_plural =  u'检索专利字段'
...
class RetrvSoftwareCRField(models.Model):
	INPUT_TYPE = 'TX'
	OPTION_TYPE = 'OP'
	RETRIEVE_TYPES = (
		(INPUT_TYPE, 'Input'),
	...
	class Meta:
		verbose_name = u'检索软件登记字段'
		verbose_name_plural =  u'检索软件登记字段'


# Create retrieve softwarecr model
scr_fields = { '__unicode__': lambda self: smart_unicode(self.name) }
try:
	for f in RetrvSoftwareCRField.objects.all():
		scr_fields[f.field_name] = models.CharField(max_length=512, verbose_name=f.field_label, blank=True, null=True)
except:
	pass
RetrvSoftwareCR = create_dynamic_model(modelname = "RetrvSoftwareCR",
					 app_label = "retrvhome",
					 module = "retrvhome.models",
					 options = {"verbose_name": u"检索软件登记", "verbose_name_plural": u"检索软件登记"},
					 admin_options = {},
					 fields = scr_fields)
